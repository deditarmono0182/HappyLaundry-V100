-- HappyLaundry Enterprise V113.0.26
-- Konfirmasi Pembayaran Online: Lunas saja vs Lunas & Ambil Barang
-- Jalankan setelah SQL 039.
...
begin
  ...
  -- "Lunas & Ambil Barang" hanya boleh saat produksi benar-benar siap.
  if coalesce(p_complete_order,false) and o.status<>'ready' then
    raise exception 'Order belum Siap Diambil. Konfirmasi Lunas saja agar status produksi tidak berubah.';
  end if;
    ...
    perform public.v100_add_payment(
      ...
      p_notes => case
        when coalesce(p_complete_order,false)
          then 'Konfirmasi pembayaran online + barang diambil pelanggan'
  ...
  -- Sangat penting:
  -- Konfirmasi Lunas biasa TIDAK menyentuh kolom status produksi.
  ...
  set
    ...
    review_note=case
      when coalesce(p_complete_order,false)
        then 'Dikonfirmasi lunas + barang diambil / selesai'
  ...
  values(
    ...
    p.id::text,
    case when coalesce(p_complete_order,false)
      then 'Pembayaran online dikonfirmasi lunas dan order ditandai selesai karena barang diambil.'
